...
```
dbx execute --cluster-name=<name of interactive cluster> --job=mlflow-cicd-ankur-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=mlflow-cicd-ankur-sample-integration-test --files-only
dbx launch --job=mlflow-cicd-ankur-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `mlflow_cicd_ankur/__init__.py` file and tag the current code version:
```
